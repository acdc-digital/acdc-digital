...
async function handleCheckoutSessionCompleted(ctx: any, session: any) {
  ...
  try {
    ...
    if (!existingUser) {
      ...
      const userWithBaseId = await ctx.runQuery(internal.shared.users.users.getUserByAuthId, {
        authId: baseAuthId
      });
      ...
      if (userWithBaseId) {
        ...
        try {
          await ctx.runMutation(internal.shared.payments.payments.updatePaymentStatus, {
        ...
        if (session.mode === "subscription") {
          ...
          if (subscriptionId) {
            ...
            await ctx.runMutation(internal.shared.users.userSubscriptions.createOrUpdate, {
...
async function handleInvoicePaymentSucceeded(ctx: any, invoice: any) {
  ...
  if (invoice.subscription) {
    ...
    const userId = await ctx.runQuery(internal.shared.users.userSubscriptions.getUserIdBySubscriptionId, {
      subscriptionId
    });
