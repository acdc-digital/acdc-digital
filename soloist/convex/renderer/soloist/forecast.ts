...
export const getSevenDayForecast = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Fetch all logs for the user in the FULL 7-day range (past + forecast days)
    // This is needed for the "Historical Logs" line which shows actual log scores for all days
    const allLogs = await ctx.runQuery(internal.renderer.soloist.forecast.getLogsForUserInRange, {
      userId,
      startDate: getISODateString(rangeStart),
      endDate: getISODateString(forecastEnd), // Include forecast days too
    });
    
    // Also fetch logs just for the past range (for backwards compatibility)
    const logs = allLogs.filter((log: any) => {
      const logDateNormalized = log.date.split('T')[0];
      return logDateNormalized >= getISODateString(rangeStart) && logDateNormalized <= getISODateString(rangeEnd);
    });
    
    console.log("[getSevenDayForecast] All logs (7-day) count:", allLogs.length);
    ...
    // Fetch historical forecasts for the ENTIRE 7-day range (for the green "Historical" line)
    // These are forecasts that were previously generated for any of the 7 days
    const allHistoricalForecasts = await ctx.db
      .query("forecast")
      .withIndex("byUserDate", (q) =>
        q.eq("userId", userId)
    ...
    console.log("[getSevenDayForecast] All historical forecasts count:", allHistoricalForecasts.length);
    console.log("[getSevenDayForecast] Historical forecast dates and scores:", 
      allHistoricalForecasts.map(f => ({ date: f.date, score: f.emotionScore }))
    );

    // Build all 4 past days (rangeStart to rangeEnd), whether logs exist or not
    const formattedPastDays = [];
    for (let i = 0; i < 4; i++) {
      const dayDate = addDays(rangeStart, i);
      const dayDateISO = getISODateString(dayDate);
      const isToday = isSameDay(dayDate, todayLocal);
      
      // Find log for this date if it exists
      const log = logs.find((l: any) => l.date.split('T')[0] === dayDateISO);
      // Find historical forecast for this date
      const historicalForecast = allHistoricalForecasts.find(f => f.date.split('T')[0] === dayDateISO);
      
      if (log) {
        // Log exists for this day
        formattedPastDays.push({
          date: log.date,
          day: getDisplayDay(dayDate, todayLocal),
          shortDay: getShortDay(dayDate),
          formattedDate: formatMonthDay(dayDate),
          emotionScore: log.score ?? null,
          actualLogScore: log.score ?? null,
          ...
          _id: log._id,
          answers: log.answers,
          historicalForecastScore: historicalForecast?.emotionScore ?? null,
        });
      } else {
        // No log for this day - show empty placeholder
        formattedPastDays.push({
          date: dayDateISO,
          day: getDisplayDay(dayDate, todayLocal),
          shortDay: getShortDay(dayDate),
          ...
          description: "No Log",
          trend: null,
          details: "No entry for this date",
          recommendation: "Log your day to see your score.",
          isPast: !isToday,
          isToday: isToday,
          isFuture: false,
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Check if there's an actual log for this forecast day (for "Historical Logs" line)
      const actualLog = allLogs.find((log: any) => log.date.split('T')[0] === forecastDateISO);
      const actualLogScore = actualLog?.score ?? null;
      // For future days, the historical forecast is the current forecast (same thing)
      forecastDays.push(existingForecast ? {
        ...
        actualLogScore: actualLogScore, // Actual log if it exists for this "future" day
        ...
        historicalForecastScore: existingForecast.emotionScore, // Same as forecast for future days
      } : {
        ...
        isPast: false, isToday: false, isFuture: true,
        confidence: 0,
        historicalForecastScore: null,
      });
    }

    // Combine past days and forecast days
    return [...formattedPastDays, ...forecastDays];
  }
});
...
export const generateRetrospectiveForecastAnalysis = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    try {
      ...
      const allLogs = await ctx.runQuery(internal.renderer.soloist.forecast.getLogsForUserInRange, allLogsQuery);
      ...
      for (let i = 4; i < Math.min(sortedLogs.length, 50); i++) { // Limit to 50 for performance
        ...
        try {
          ...
          const forecastResult = await ctx.runAction(internal.renderer.soloist.generator.generateForecastWithAI, {
            userId,
...
export const generateForecast = action({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; error?: string; forecastDates?: string[] }> => {
    ...
    const logs = await ctx.runQuery(internal.renderer.soloist.forecast.getLogsForUserInRange, {
      userId,
      startDate: getISODateString(rangeStart),
    ...
    const forecasts = await ctx.runAction(internal.renderer.soloist.generator.generateForecastWithAI, {
      userId,
    ...
    for (const forecast of forecasts) {
      await ctx.runMutation(internal.renderer.soloist.forecast.deleteExistingForecast, {
        userId,
        date: forecast.date,
      });
      await ctx.runMutation(internal.renderer.soloist.forecast.insertForecast, {
        userId,
        date: forecast.date,
