...
import { SCORING_PROMPT } from "../../shared/lib/prompts";
...
export const scoreDailyLogNew = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const dailyLog = await ctx.runQuery(api.renderer.heatmap.dailyLogs.getDailyLog, {
        userId,
        date,
      });
      ...
      await ctx.runMutation(api.renderer.heatmap.score.updateLogScore, {
        logId: dailyLog._id,
        newScore: score,
      });
      ...
      try {
        ...
        const feedResult = await ctx.runAction(api.renderer.heatmap.feed.generateFeedForDailyLog, {
          userId,
          date,
        });
...
export const scoreDailyLog = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (completion.usage) {
        try {
          await ctx.runMutation(internal.website.admin.anthropic.trackUsage, {
            userId,
            feature: "scoring",
            model: "claude-3-5-haiku-20241022",
