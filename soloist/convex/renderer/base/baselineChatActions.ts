...
export const generateBaselineAnalysis = action({
  ...
  handler: async (ctx, args) => {
    ...
    const baselineAnswers = await ctx.runQuery(internal.renderer.base.baselineChat.getBaselineAnswers, {
    ...
    const baseline = await ctx.runQuery(internal.renderer.base.baseline.getLatestBaseline, {
      userId: args.userId,
    });
    ...
    // Track usage
    await ctx.runMutation(internal.renderer.base.baselineChat.trackAnthropicUsage, {
      userId: args.userId,
    ...
    await ctx.runMutation(api.renderer.base.baselineChat.addChatMessage, {
      userId: args.userId,
      baselineAnswerId: args.baselineAnswerId,
      role: "assistant",
      content: analysis,
...
export const chatWithAnalysis = action({
  ...
  handler: async (ctx, args) => {
    ...
    const recentMessages = await ctx.runQuery(api.renderer.base.baselineChat.getRecentMessages, {
      baselineAnswerId: args.baselineAnswerId,
      limit: 4,
    });
