...
// /Users/matthewsimon/Projects/AURA/AURA/app/_components/dashboard/_components/calendarTab/DashCalendarConsole.tsx
...
type PlatformFilter = 'all' | 'twitter' | 'instagram' | 'linkedin' | 'youtube';
...
export function DashCalendarConsole() {
  ...
  // Calendar store for view controls
  const { 
    currentView, 
    setCurrentView,
    openCreatePostModal 
  ...
  // Mock scheduled posts hook for demo
  const useScheduledPostsFromConvex = () => ({
  ...
  const normalizePost = (post: ConvexPost): CalendarPost => ({
    _id: post._id,
  ...
  const thisWeekPosts = sortedPosts.filter(post => post.scheduledAt >= tomorrowTime && post.scheduledAt < weekTime);
        ...
        <div className="space-y-1 mt-2">
                    ...
                    return (
                      <div key={filter.id} className="flex items-center justify-between px-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => setSelectedFilter(filter.id as PlatformFilter)}
          ...
          <div className="rounded bg-[#1e1e1e] border border-[#2d2d2d]">
            <button
              ...
              className="w-full flex items-center gap-2 p-2 hover:bg-[#2d2d2d]/30 transition-colors"
            >
              {expandedSections.has('viewControls') ? 
                <ChevronDown className="w-3.5 h-3.5 text-[#858585]" /> : 
                <ChevronRight className="w-3.5 h-3.5 text-[#858585]" />
              }
              <Calendar className="w-3.5 h-3.5 text-[#858585]" />
              <span className="text-xs font-medium flex-1 text-left">Calendar View</span>
              <div className="flex items-center gap-1">
                <span className="text-xs text-[#cccccc] capitalize">{currentView}</span>
              </div>
            </button>
            ...
            {expandedSections.has('viewControls') && (
              <div className="px-2 pb-2 space-y-2">
                ...
                {/* View Selection */}
                <div className="space-y-1">
                  <div className="text-xs text-[#858585] mb-1 px-1">Calendar Layout</div>
                  
                  {[
                    { id: 'week', label: 'Week View', icon: Calendar },
                    { id: 'month', label: 'Month View', icon: Calendar },
                    { id: 'day', label: 'Day View', icon: Clock },
                    { id: 'agenda', label: 'Agenda View', icon: MessageSquare },
                  ].map((view) => {
                    ...
                    return (
                      <div key={view.id} className="flex items-center justify-between px-1">
                        <div className="flex items-center gap-2">
                          <Icon className="w-3 h-3 text-[#858585]" />
                          <span className="text-xs text-[#858585]">{view.label}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setCurrentView(view.id as 'week' | 'month' | 'day' | 'agenda')}
                            className={`text-xs underline-offset-2 hover:underline ${
                              isActive 
                                ? 'text-[#007acc] font-medium' 
                                : 'text-[#858585] hover:text-[#cccccc]'
