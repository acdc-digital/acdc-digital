...
`);
  ...
  private async printDetailedReport(
    ...
    this.printDetailedScore(
      "Separation of Concerns",
      scores.separationOfConcerns,
    );
    this.printDetailedScore("Maintainability", scores.maintainability);
    this.printDetailedScore(
      "Dependency Management",
      scores.dependencyManagement,
    );
    ...
    if (result.violations.length > 0) {
      ...
      result.violations.forEach((violation, index: number) => {
        ...
        console.log(
          `${index + 1}. ${severityIcon} ${violation.file}:${violation.line}`,
        );
        console.log(`   ${violation.violation}`);
        console.log(
          `   Category: ${violation.category}, Principle: ${violation.principle}\n`,
        );
      });
    }
  }

  private printDetailedScore(
    name: string,
    score: StructureAuditResult["scores"]["modularity"],
  ): void {
    const scoreColor =
      score.score >= 4
        ? "\x1b[32m"
  ...
  private async generateMermaidFiles(
    result: StructureAuditResult,
  ): Promise<void> {
    ...
    console.log(
      "1. Open the .mmd files in VS Code with Mermaid Preview extension",
    );
    console.log(
      "2. Or copy the content to https://mermaid.live for online viewing",
    );
  }

  private async printOrganizationNodes(
    result: StructureAuditResult,
  ): Promise<void> {
    ...
    // Group nodes by type
    const nodesByType = result.organizationNodes.reduce(
      (acc: Record<string, typeof result.organizationNodes>, node) => {
    ...
    for (const [type, nodes] of Object.entries(nodesByType)) {
      ...
      const sortedNodes = nodes.sort(
        (a, b) => b.maintainabilityIndex - a.maintainabilityIndex,
      );

      for (const node of sortedNodes.slice(0, 10)) {
        // Show top 10
        const maintainabilityColor =
          node.maintainabilityIndex >= 70
            ? "\x1b[32m"
            : node.maintainabilityIndex >= 50
              ? "\x1b[33m"
              : "\x1b[31m";
        ...
        console.log(
          `    Complexity: ${node.complexity}, Maintainability: ${node.maintainabilityIndex}%`,
        );
        console.log(
          `    Dependencies: ${node.dependencies.length}, Dependents: ${node.dependents.length}`,
        );
  ...
  private async printDependencyLadders(
    result: StructureAuditResult,
  ): Promise<void> {
    ...
    for (const ladder of result.dependencyLadders) {
      ...
      for (const level of ladder.levels) {
        ...
        level.components
          .slice(0, 10)
          .forEach((component: string, index: number) => {
