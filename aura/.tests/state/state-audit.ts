...
class StateAuditor {
  ...
  async auditCompleteArchitecture(): Promise<AuditResult> {
    ...
    const dataFlowAudit = await this.auditDataFlowArchitecture(
      componentFiles,
      hookFiles,
    );
  ...
  private async auditAuthSynchronization(): Promise<AuthSyncAuditResult> {
    ...
    const authSyncPath = path.join(
      this.projectRoot,
      "app/_components/auth/AuthSync.tsx",
    );
    ...
    if (!hasAuthSync) {
      this.violations.push({
        ...
        line: 1,
        violation:
          "Missing AuthSync component for Clerk/Convex synchronization",
        severity: "error",
    ...
    if (fs.existsSync(layoutPath)) {
      ...
      isInLayout =
        layoutContent.includes("AuthSync") ||
        layoutContent.includes("<AuthSync");
    ...
    if (fs.existsSync(useUserPath)) {
      ...
      const hasBridgeLogic =
        hookContent.includes("clerkUser") &&
        hookContent.includes("getCurrentUser");

      hasConvexIntegration =
        hasClerkImport && hasConvexImport && hasBridgeLogic;
      ...
      if (!hookContent.includes('"skip"')) {
        this.violations.push({
          ...
          line: 1,
          violation:
            "Missing conditional queries to prevent authentication errors",
          severity: "warning",
          principle: "Authentication Synchronization",
          category: "auth-sync",
        });
  ...
  private async auditDataFlowArchitecture(
    componentFiles: string[],
    _hookFiles: string[],
  ): Promise<DataFlowAuditResult> {
    ...
    for (const filePath of componentFiles) {
      ...
      if (
        relativePath.includes("UserProfile.tsx") ||
        relativePath.includes("AuthSync.tsx")
      ) {
        continue;
      }

      const directConvexCalls =
        (content.match(/useQuery\(api\./g) || []).length +
      ...
      lines.forEach((line, index) => {
        if (line.includes("useState") && this.containsBusinessData(line)) {
          ...
          this.violations.push({
            ...
            line: index + 1,
            violation:
              "Business data in useState. Use Convex for persistent data.",
            severity: "error",
    ...
    for (const filePath of storeFiles) {
      ...
      if (this.containsBusinessDataInStore(content)) {
        ...
        this.violations.push({
          ...
          line: 1,
          violation:
            "Zustand store contains business data. Use only for UI state.",
          severity: "error",
    ...
    return {
      convexAsSourceOfTruth,
      zustandForUIOnly,
      customHooksPattern,
      noDirectConvexInComponents,
    };
  }
  ...
  private async auditStateSeparation(componentFiles: string[]): Promise<void> {
    ...
    for (const filePath of componentFiles) {
      ...
      if (
        ...
        this.containsBusinessData(content)
      ) {
        this.violations.push({
          file: relativePath,
          line: 1,
          violation:
            "Component mixes server state (Convex) with local state (useState) for business data",
          severity: "warning",
    ...
    for (const filePath of allFiles) {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes("Id<") && !line.includes("import")) {
          ...
          !relativePath.includes("schema.ts")
        ) {
          this.violations.push({
            file: relativePath,
            line: index + 1,
            violation:
              'Consider using Convex Id<"users"> instead of string for user IDs',
            severity: "warning",
            principle: "Type Safety",
            category: "type-safety",
          });
    ...
    for (const filePath of [...componentFiles, ...hookFiles]) {
      ...
      if (useEffectMatches) {
        useEffectMatches.forEach((match) => {
          if (
            match.includes("[]") &&
            (match.includes("user") || match.includes("clerkUser"))
          ) {
            this.violations.push({
              file: relativePath,
              line: 1,
              violation:
                "useEffect with empty deps array but references user data - potential stale closure",
              severity: "warning",
      ...
      if (
        ...
        this.violations.push({
          ...
          line: 1,
          violation:
            "Consider using useMemo for expensive computations with query results",
          severity: "warning",
          principle: "Performance",
          category: "performance",
        });
  ...
  private printEnhancedResults(result: AuditResult): void {
    ...
    console.log(
      `   🎯 Overall: ${Math.round(result.summary.overallScore)}/100`,
    );
    ...
    console.log(
      `   AuthSync Component: ${result.authSyncAudit.hasAuthSync ? "✅" : "❌"}`,
    );
    console.log(
      `   Layout Integration: ${result.authSyncAudit.isInLayout ? "✅" : "❌"}`,
    );
    console.log(
      `   User Hook Bridge: ${result.authSyncAudit.hasUserHook ? "✅" : "❌"}`,
    );
    console.log(
      `   Convex Integration: ${result.authSyncAudit.hasConvexIntegration ? "✅" : "❌"}`,
    );

    // Data flow details
    console.log(`\n🌊 DATA FLOW ARCHITECTURE:`);
    console.log(
      `   Convex as Source of Truth: ${result.dataFlowAudit.convexAsSourceOfTruth ? "✅" : "❌"}`,
    );
    console.log(
      `   Zustand UI-Only: ${result.dataFlowAudit.zustandForUIOnly ? "✅" : "❌"}`,
    );
    console.log(
      `   Custom Hooks Pattern: ${result.dataFlowAudit.customHooksPattern ? "✅" : "❌"}`,
    );
    console.log(
      `   No Direct Convex in Components: ${result.dataFlowAudit.noDirectConvexInComponents ? "✅" : "❌"}`,
    );

    // Violations by category
    if (result.violations.length > 0) {
      const violationsByCategory = result.violations.reduce(
        (acc: Record<string, StateViolation[]>, violation) => {
      ...
      console.log(`\n🚨 VIOLATIONS BY CATEGORY:`);
      Object.entries(violationsByCategory).forEach(
        ([category, violations]: [string, StateViolation[]]) => {
          const errors = violations.filter(
            (v: StateViolation) => v.severity === "error",
          ).length;
          const warnings = violations.filter(
            (v: StateViolation) => v.severity === "warning",
          ).length;
          console.log(
          ...
          violations.forEach((violation: StateViolation) => {
            ...
            console.log(
              `     ${icon} ${violation.file}:${violation.line} - ${violation.violation}`,
            );
          });
        },
      );
