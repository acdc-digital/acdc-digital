...
import { X, Plus, Code, FileText, Database, MapPin } from 'lucide-react'
import { RunningDashboard } from '../activity/_components/health/RunningDashboard'
...
export const Dashboard: FC<DashboardProps> = ({ className }) => {
  const { tabs, activeTabId, setTabs, setActiveTabId, closeTab, addRunningTab } = useDashboardStore();
  
  // Initialize with default tabs if store is empty
  useEffect(() => {
    if (tabs.length === 0) {
