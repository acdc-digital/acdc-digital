...
interface Tab {
  id: string;
  title: string;
  type: 'file' | 'welcome' | 'settings' | 'subscription' | 'user-profile' | 'calendar' | 'books' | 'todos' | 'running' | 'research';
  filePath?: string;
  isDirty: boolean;
  isPinned: boolean;
}
...
export const useEditorStore = create<EditorState>()(
  persist(
    (set) => ({
      ...
      openRunningTab: () => set((state) => {
        const runningTabId = `running-${Date.now()}`;
        const newTab: Tab = {
          id: runningTabId,
