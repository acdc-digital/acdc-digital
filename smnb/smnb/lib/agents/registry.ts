// AGENT REGISTRY - SMNB Project
// /Users/matthewsimon/Projects/acdc-digital/smnb/smnb/lib/agents/registry.ts

/**
 * SMNB Agent Registry
 * 
 * Central registry for all SMNB agents following the AURA agent pattern
 */

import { BaseAgent, ConvexMutations, AgentExecutionContext, AgentExecutionResult } from './base';

export class SMNBAgentRegistry {
  ...
  register(agent: BaseAgent): void {
    ...
    console.log(`✅ SMNB Agent registered: ${agent.name} (${agent.id})`);
  ...
  async executeCommand(
    ...
    try {
      ...
      console.log(`🤖 SMNB Executing ${agent.name} - ${tool.name}: ${input.substring(0, 50)}...`);
      ...
      console.error(`❌ SMNB Agent execution error:`, error);
...
// Global SMNB registry instance
export const smnbAgentRegistry = new SMNBAgentRegistry();