...
export type ClaudeModel = "claude-haiku-4-5-20251001";
...
export const SENTIMENT_ANALYSIS: LLMPromptConfig<SentimentAnalysisInput> = {
  ...
  description: "Generate 1-paragraph sentiment analysis excerpts",
...
export const NEWS_SUMMARY: LLMPromptConfig<NewsSummaryInput> = {
  ...
  systemPrompt: `You are a financial news analyst specializing in market intelligence and news synthesis. 
Your task is to analyze recent news coverage and provide clear, data-driven insights about 
company developments, market positioning, and investor implications.`,
  ...
  buildPrompt: (input: NewsSummaryInput) => {
    ...
    return `Analyze the following news articles for ${ticker}:
...
Generate a professional markdown-formatted analysis with the following structure:

# ${ticker} News Analysis

**Recent Developments:**
Synthesize the key news stories and developments (50-75 words). Focus on what's actually happening—product launches, earnings reports, strategic moves, regulatory news, or market positioning changes. Be specific about timelines and significance.

**Market Implications & Outlook:**
Analyze what these developments mean for investors and the company's trajectory (50-75 words). Discuss potential impacts on valuation, competitive position, or growth prospects. Include relevant context about industry trends or analyst perspectives if mentioned in the articles.

IMPORTANT:
- Use markdown formatting (## headers, **bold** for emphasis)
- Keep total length to 100-150 words
- Be objective and data-driven
- Focus on material information that affects investment thesis
- Avoid speculation—stick to facts from the articles
- Use professional but accessible language

Then on a new line output:
SENTIMENT_SCORE: [number from 0-100 based on news tone]`;
  },

  outputFormat: "Markdown-formatted analysis with SENTIMENT_SCORE at the end"
};
